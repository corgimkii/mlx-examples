...
"""
Utility functions for Wan2.2 I2V-A14B pipeline.

Weight loading, HF Hub downloading, video saving.

Compared to wan2.1's `utils.py`, this variant carries two DiT checkpoints
(`high_noise_model` / `low_noise_model`) under the same HF repo, plus a
`boundary` timestep that selects which one runs at any given step.
"""
...
class ModelSpec:
    repo_id: str
    # The A14B MoE ships two DiT checkpoints (high_noise / low_noise) selected
    # by timestep boundary; both live under one HF repo as nested directories
    # with their own safetensors shards + index.
    repo_dit_high: str
    repo_dit_low: str
    repo_vae: str
    repo_t5: str
    dit_params: Dict
    # Wan2.2 reference: t * num_train_timesteps boundary; t >= boundary uses
    # the high-noise expert, otherwise low-noise.
    boundary: float
    repo_tokenizer: str = "google/umt5-xxl/tokenizer.json"
    ckpt_path_high: Optional[str] = None
    ckpt_path_low: Optional[str] = None


configs = {
    "i2v-A14B": ModelSpec(
        repo_id="Wan-AI/Wan2.2-I2V-A14B",
        repo_dit_high="high_noise_model/diffusion_pytorch_model.safetensors.index.json",
        repo_dit_low="low_noise_model/diffusion_pytorch_model.safetensors.index.json",
        ...
        ckpt_path_high=os.getenv("WAN_I2V_A14B_HIGH"),
        ckpt_path_low=os.getenv("WAN_I2V_A14B_LOW"),
    ),
}
...
def load_dit(name: str, expert: str, checkpoint: Optional[str] = None) -> WanModel:
    """Load a DiT expert with weights from HF Hub.

    expert: 'high' or 'low'. The A14B MoE has two DiTs selected by timestep
        boundary; both share the same `dit_params` but are stored as separate
        sharded checkpoints under the same HF repo.
    """
    ...
    if checkpoint is None:
        ckpt_path = spec.ckpt_path_high if expert == "high" else spec.ckpt_path_low
        if ckpt_path is None:
            repo_file = spec.repo_dit_high if expert == "high" else spec.repo_dit_low
            ckpt_path = _hf_download(spec.repo_id, repo_file)
    else:
